...
export default function CheckLeaves({ toast }) {
  ...
  const headers = ["Leave Id", "Nature", "Name","Position","Start Date", "Status"];  
  ...
  const fetchLeaves = async (e) => {
    try {
      ...
      for (let i = 0; i < data.length; i++) {
        temp.push([data[i].id, data[i].nature, data[i].name,data[i].position, data[i].start_date.slice(0, -12), data[i].status]);
      }
      setData(temp);
      setLeaves(data)
    } catch (error) {
