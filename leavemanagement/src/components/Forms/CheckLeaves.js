...
<<<<<<< HEAD
	...
	const fetchLeaves = async (e) => {
		try {
			const resp = await httpClient.post(
				`${process.env.REACT_APP_API_HOST}/check_applications`
			);
	...
	const fetchNumberOfLeaves = async (e) => {
		try {
			const resp = await httpClient.post(
				`${process.env.REACT_APP_API_HOST}/fetch_number_of_leaves`
			);
			if (resp.data.status == "success") {
				// toast.success("Leaves fetched Successfully", toast.POSITION.BOTTOM_RIGHT);
			} else {
...
>>>>>>> da8b26507be2715f671374eb4d6f9471d2ef293b
}
